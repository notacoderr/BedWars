...
class BedWars extends PluginBase implements Listener{
    ...
    public $prefix = TextFormat::GRAY."[".TextFormat::AQUA."BedWars".TextFormat::GRAY."]".TextFormat::WHITE." ";
    public $Signprefix = TextFormat::GRAY."[". TextFormat::AQUA . "BedWars" . TextFormat::GRAY."]" . TextFormat::WHITE." ";
    ...
    public function createArena($arena, $teams, $ppt){
        ...
        $this->getLogger()->info(TextFormat::GREEN."Runde".TextFormat::AQUA.$arena.TextFormat::GREEN." wurde Erfolgreich erstellt!");
    }
    ...
    public function onRespawn(PlayerRespawnEvent $event){
        ...
        if($this->inArena($player)){
            ...
            if($config->getNested("Bed.".$team.".Alive") == true){
                ...
                $player->sendMessage($this->prefix.TextFormat::RED."Dein Bett wurde zerstÃÂ¶rt, du kannst nicht mehr respawnen!");
                ...
                if ($cause instanceof EntityDamageByEntityEvent) {
                    ...
                    if ($killer instanceof Player) {
                        foreach ($players as $pn) {
                            $p = $this->getServer()->getPlayerExact($pn);
                            if($p != null) {
                                $p->sendMessage($this->prefix . $killer->getNameTag() . TextFormat::GRAY. " hat " . $player->getNameTag() . TextFormat::GRAY . " getÃÂ¶tet!");
                            }
                        }
                    } else {
                        foreach ($players as $pn) {
                    ...
                    foreach ($players as $pn) {
                        ...
                        if($p != null) {
                            ...
                            if($this->lasthit[$player->getName()] != "no"){
                                ...
                                if($p2 != null){
                                    $p->sendMessage($this->prefix . $p2->getNameTag() . TextFormat::WHITE. " hat " . $player->getNameTag() . TextFormat::WHITE . " getÃÂ¶tet!");
                                    $this->lasthit[$player->getName()] = "no";
                                } else {
                                    $p->sendMessage($this->prefix . $player->getNameTag() . TextFormat::GRAY . " ist gestorben!");
                                }
                        ...
                        foreach ($this->getPlayers($arena) as $pn) {
                            ...
                            if ($p != null) {
                                if ($team == $this->getTeam($p->getNameTag())) {
                                    $p->sendMessage($this->prefix . TextFormat::RED . "Das Bett von deinem Team wurde zerstÃ¶rt!");
                                } else {
                                    $p->sendMessage($this->prefix . "Das Bett von Team " . TextFormat::GOLD . $team . TextFormat::WHITE . " wurde zerstÃ¶rt!");
                                }

                            }
                        }
                    } else {
                        $player->sendMessage($this->prefix . "Du kannst dein eigenes Bett nicht zerstÃ¶ren!");
                        $event->setCancelled();
                    }
                }
    ...
    public function onInteract(PlayerInteractEvent $event){
        ...
        if($this->registerBed == true && $this->registerBedWHO == $name){
            ...
            $player->sendMessage(TextFormat::GREEN . "Du hast Erfolgreich das Bett von Team " . TextFormat::AQUA . $team . TextFormat::GREEN . " fÃÂ¼r die Arena " . TextFormat::AQUA . $arena . TextFormat::GREEN . " registriert!");
        ...
        if($cmd->getName() == "Bedwars" && $sender->isOP()){
            if(!empty($args[0])){
                if(strtolower($args[0]) == "help" && $sender->isOP()){
                    ...
                    $sender->sendMessage(TextFormat::GRAY."-> ".TextFormat::DARK_AQUA."/bw addarena <ArenaName> <Teams> <SpielerProTeam> ".TextFormat::GRAY."[".TextFormat::RED."FÃÂ¼gt eine neue Arena hinzu".TextFormat::GRAY."]");
                ...
                elseif(strtolower($args[0]) == "setlobby" && $sender->isOP()){
                    if(!empty($args[1])) {
                        ...
                        if($this->arenaExists($arena)) {
                            ...
                            $sender->sendMessage(TextFormat::GREEN . "Du hast Erfolgreich die Lobby fÃÂ¼r die Arena " . TextFormat::AQUA . $arena . TextFormat::GREEN . " gesetzt!");
                            $sender->sendMessage(TextFormat::GREEN . "Setup -> /bw help");
                    ...
                    if(!empty($args[1]) && !empty($args[2])) {
                        ...
                        if($this->arenaExists($arena)) {
                            if (in_array($team, $this->getAllTeams())) {
                                ...
                                $sender->sendMessage(TextFormat::GREEN . "Tippe nun ein Bett an! bitte Tippe die untere HÃÂ¤lfte an da es sonst zu bugs kommen kann!");
                    ...
                    if(!empty($args[1]) && !empty($args[2])) {
                        ...
                        if($this->arenaExists($arena)) {
                            if (in_array($team, $this->getAllTeams())) {
                                ...
                                $sender->sendMessage(TextFormat::GREEN . "Du hast Erfolgreich den Spawn des Teams ".TextFormat::AQUA . $team . TextFormat::GREEN." fÃÂ¼r die Arena " . TextFormat::AQUA . $arena . TextFormat::GREEN . " gesetzt!");
                                $sender->sendMessage(TextFormat::GREEN . "Setup -> /bw help");
